...
CREATE TABLE `jobs` (
  ...
  `errors` text NULL,
  `task_jobs` uuid NOT NULL,
  PRIMARY KEY (`id`),
  CONSTRAINT `jobs_tasks_jobs` FOREIGN KEY (`task_jobs`) REFERENCES `tasks` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE
);
...
CREATE TABLE `job_logs` (
  ...
  `size` integer NULL,
  `job_logs` uuid NOT NULL,
  CONSTRAINT `job_logs_jobs_logs` FOREIGN KEY (`job_logs`) REFERENCES `jobs` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE
);
-- create "tasks" table
CREATE TABLE `tasks` (
  `id` uuid NOT NULL,
